...
Una vez contabilizadas las frecuencias totales, el programa calcula la probabilidad `p_i` de cada símbolo (frecuencia del símbolo / total de bytes) y aplica la sumatoria:

$$H = -\sum p_i \log_2(p_i)$$

4. **Cálculo de Redundancia:**
La entropía máxima teórica para un alfabeto de 256 símbolos equiprobables es log₂(256) = 8 bits/símbolo. La redundancia absoluta de los datos se calcula restando la entropía empírica obtenida del máximo teórico (8 - H).
...
* Tamaño: 14,679 bytes
* Entropía Empírica: 4.5406 bits/símbolo
* Redundancia: 3.4594 bits/símbolo

**2. Archivo Comprimido (`texto_prueba.zip`)**

* Tamaño: 6,338 bytes (Nota: el tamaño del zip en el IC anterior era de 6338, asumo que el script de entropía se corrió sobre el mismo archivo .zip, pero no hay imagen. Se usarán los valores del nuevo IC y los nuevos valores de entropía de la imagen)
* Entropía Empírica: (Falta el valor de entropía del zip nuevo en las capturas, se deja en blanco para que lo completes o se calcula si es necesario) 
* Redundancia: (Falta el valor de redundancia del zip nuevo)
...
* **El archivo de texto plano** posee una entropía baja (4.54 bits/símbolo) y una alta redundancia (casi 3.46 bits/símbolo desperdiciados). Esto se debe a la naturaleza de los lenguajes naturales: ciertas letras (como las vocales o el espacio en blanco) tienen una frecuencia de aparición muchísimo mayor que otras. Esta distribución de probabilidad tan desigual genera "huecos" que el algoritmo de compresión aprovecha.
* **El archivo `.zip`**, por el contrario, reduce el tamaño del archivo al eliminar la redundancia estadística. El algoritmo agrupa los patrones repetitivos y los sustituye. Al hacer esto, los bytes resultantes en el archivo comprimido presentan una distribución de probabilidad mucho más uniforme.
* **Conclusión:** A medida que se elimina la redundancia en los datos, la entropía empírica se acerca al límite teórico máximo (8 bits/símbolo). En el archivo comprimido, la incertidumbre por byte aumenta significativamente; es decir, cada byte en el `.zip` aporta mayor "información pura" estadísticamente impredecible en comparación con el texto original.