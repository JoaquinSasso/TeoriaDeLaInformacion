...
# Semilla maestra para generar la matriz del canal
...
def atender_cliente(cliente, direccion):
    ...
    # Esto asegura que cada sesion sea 100% determinista.
    ...
    try:
        while True:
            ...
            if any(bit not in "01" for bit in mensaje):
                enviar_mensaje(
                    cliente,
                    "ERROR: solo se permiten simbolos 0 y 1"
                )
                continue
            ...
            for bit in mensaje:
                # Si el numero aleatorio cae dentro de la
                # probabilidad de error, invertimos el bit.
